...
test_that("Error if opts is missing ", {
  expect_error(kl(data = HOT, choice = choice, basis = "log2"))
})

test_that("Error if opts has just length 1", {
  expect_error(kl(data = HOT, opts = Option_1, choice = choice, basis = "log2"))
})
...
test_that("Warning if group contains NA ", {
  ...
  expect_warning(kl(data = HOT2, opts = c(Option_1:None), choice = choice, basis = "log2", group = Group))
})
...
test_that("Structure of Output data.frame ", {
  expect_true(base::is.data.frame(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2")))
})

test_that("Structure of Output tibble ", {
  expect_true(tibble::is_tibble(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2")))
})

test_that("Length of output equals number of groups - no group ", {
  expect_equal(base::nrow(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2")), 1)
})

test_that("Length of output equals number of groups - 1 group ", {
  expect_equal(base::nrow(kl(data = HOT, opts = c(Option_1:None), choice = choice, group = Group, basis = "log2")), base::length(base::unique(HOT$Group)))
})

test_that("Length of output equals number of groups - 2 group ", {
  HOT$Group2 <- c("Group 1", "Group 2")
  expect_equal(base::nrow(kl(data = HOT, opts = c(Option_1:None), basis = "log2", choice = choice, group = c(Group, Group2))), (base::length(base::unique(HOT$Group)) * base::length(base::unique(HOT$Group2))))
})

test_that("Always two values ", {
  expect_equal(base::ncol(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2")), 2)
})

test_that("Check column names ", {
  expect_equal(colnames(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2")), c("kl_o_p", "kl_p_o"))
})

test_that("Check missing symmetry ", {
  expect_true(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2")[[1]] != kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2")[[2]])
  expect_true(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log")[[1]] != kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log")[[2]])
})

test_that("Numeric output - no group ", {
  expect_true(base::is.numeric(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2")[[1]]))
  expect_true(base::is.numeric(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2")[[2]]))
})

test_that("Numeric output - 1 group ", {
  expect_true(base::is.numeric(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2", group = Group)[[2]]))
  expect_true(base::is.numeric(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2", group = Group)[[3]]))
})

test_that("group output equals group input ", {
  expect_equal(utils::str(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2")[[1]]), utils::str(HOT$Group))
})

test_that("group output equals group input - character input ", {
  HOT$Group2 <- c("Group 1", "Group 2")
  expect_equal(utils::str(kl(data = HOT, opts = c(Option_1:None), choice = choice, group = Group2, basis = "log2")[[1]]), utils::str(HOT$Group2))
})
...
test_that("group output equals group input - multiple grouping variables ", {
  ...
  expect_true(base::is.numeric(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2", group = c(Group, Group2))[[3]]))
  expect_true(base::is.numeric(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2", group = c(Group, Group2))[[4]]))
})
...
test_that("kl() also working with data.frame not created with createHOT()", {
  ...
  expect_true(base::is.numeric(kl(data = newHOT, opts = c(Option_1:Option_5), choice = Choice, basis = "log2")[[1]]))
  expect_true(tibble::is_tibble(kl(data = newHOT, opts = c(Option_1:Option_5), choice = Choice, basis = "log2")))
  expect_false(base::anyNA(kl(data = newHOT, opts = c(Option_1:Option_5), choice = Choice, basis = "log2")))
})

test_that("check whether examples are correct ", {
  expect_equal(base::round(base::as.numeric(kl(data = HOT, opts = c(Option_1:None), choice = choice, basis = "log2")), 2), c(0.18, 0.17))
  expect_equal(base::round(base::as.numeric(kl(data = HOT, opts = c(Option_1:None), choice = choice, group = Group, basis = "log2")[[2]]), 2), c(1.09, 0.23, 0.60))
  expect_equal(base::round(base::as.numeric(kl(data = HOT, opts = c(Option_1:None), choice = choice, group = Group, basis = "log2")[[3]]), 2), c(0.29, 0.61, 1.34))
})
