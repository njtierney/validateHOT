...
```{r}
kl(data = HOT, id = 1, opts = c(2:9), choice = 11)
```
### `mae`
...
In case, we first had change `Group` to a `factor`, the output would have been adapted accordingly. This is shown in the example in which we use `mae` in `tidyverse` logic. To do so, we load the `magrittr` (Bache & Wickham, 2022) and `dplyr` (Wickham et al., 2023) package.
