...
So far, we have only looked at examples where at maximum only one attribute was not part-worth coded. In the final example, we will have all three types of coding included. 
Since piece-wise coding is only available for ACBC in Sawtooth Software, we will show another example with an ACBC. The data set is similar to the one from the previous example (6 alternatives plus a no-buy alternative). We will use the data frame `ACBC_interpolate`, which is also part of `validateHOT`. 
