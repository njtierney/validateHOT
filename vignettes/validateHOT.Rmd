...
```{r}
HOT %>%
  kl(data = ., opts = c(Option_1:None), choice = choice, basis = "log")
```
...
We begin with the accuracy and we still work with the `HOT` data frame we created above. We specify the 8 alternatives (7 alternatives + no-buy alternative) and finally `None`. 
...
HOT %>%
  ...
  recall(data = ., opts = c(Option_1:None), choice = choice, none = None, group = Group) %>%
...
```{r}
reach(data = HOT, opts = c(Option_1, Option_4, Option_6), none = None)
```
...
```{r}
freqassort(data = HOT, opts = c(Option_1, Option_4, Option_6), none = None)
```

Again, we could use the tidyverse logic and get the results by group

```{r}
HOT %>%
  dplyr::mutate(Group = factor(Group, levels = c(1:3), labels = paste0("Group ", c(1:3)))) %>%
  freqassort(data = ., opts = c(Option_1, Option_4, Option_6), none = None, group = Group) %>%
  dplyr::mutate_if(is.numeric, round, 2)
```
