...
In case you have a forced-choice validation/holdout task, do not specify the `none` argument. Finally, in case you want to create a market scenario and not a validation/holdout task, you do not need to define the `choice` argument.
...
Finally, in our example, we would also like to keep some variables that should be attached to the data frame we are creating. We can specify the variables we would like to keep in `varskeep`.
...
> [**Important**]{.underline}: Remember, since we have a linear-coded variable included, we **have** to provide the column indexes in `prod.levels`. For the other arguemnts (in the case above `id`, `none`, `lin.p`, `varskeep`, and `choice`) we could still specify the column names.
...
So far, we have only looked at examples where at maximum one attribute was not part-worth coded. In the final example, we will have all three types of coding included. Since piecewise coding is only available for ACBC in Sawtooth Software, we will show another example with an ACBC. The data set is similar to the previous one (six alternatives plus a no-buy alternative). We will use the data frame `ACBC_interpolate`, which is also part of `validateHOT`.
...
Next, we assume that one of the attributes was linear-coded (`Att3_Lin`), which we specify in `coding`. Again, as we had to do for `createHOT()` as well, we need to provide the values that we used for model estimation (see above). We again specify those in the argument `interpolate.levels`.
...
Above, we explained how to address a piecewise-coded ACBC in `createHOT()`. We again specify a linear-coded price in ACBC as piecewise-coded (see explanation above).
