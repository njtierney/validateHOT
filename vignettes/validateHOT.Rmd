...
Moreover, we also load two packages for later purposes. We also explain some functions using the  `tidyverse` logic. To do so, we load the `magrittr` (Bache & Wickham, 2022) and `dplyr` (Wickham et al., 2023) package. However, please be aware that if you specify a group, do **not** use the refer to the `Group` command in the according function. 
...
```{r}
hitrate(data = HOT, id = 1, opts = c(2:9), choice = 11)
```
...
```{r}
kl(data = HOT, id = 1, opts = c(2:9), choice = 11, basis = "log2")
```

However, we can also use the normal $log$. Let us try this in the next step and this time we display the results by group using the `tidyverse` logic.

```{r}
HOT %>% 
  kl(data = ., id = 1, opts = c(2:9), choice = 11, basis = "log", Group = 10)
```
...
In case, we first had change `Group` to a `factor`, the output would have been adapted accordingly. 
...
Next, we will take a look at the 5 metrics from confusion matrix. The current logic for the confusion matrix is that `validateHOT` calculates, e.g., whether your model overestimates or underestimates purchase behavior. Therefore, you need to specify a `None` for the 5 metrics of confusion matrix, namely ...
...
The setup is similar to the ones from the validation metrics reported above, the only difference is that you have to specify the column index of the no-buy alternative (`None`).

### `accuracy`

We begin with the accuracy and we still work with the `HOT` data frame we created above. We specify the 8 alternatives (7 alternatives + no-buy alternative) and finally the column index of `None`. 

```{r}
accuracy(data = HOT, id = 1, opts = c(2:9), choice = 11, None = 9)
```

### `f1`

Calculating the *F1*-score is exactly the same.

```{r}
f1(data = HOT, id = 1, opts = c(2:9), choice = 11, None = 9)
```

### `precision`

For `precision`, we will specify the `Group` argument again.

```{r}
precision(data = HOT, id = 1, opts = c(2:9), choice = 11, None = 9, Group = 10)
```


### `recall``

Similar to the validation metrics shown above, we can also implement these functions in `tidyverse` logic.

```{r}
HOT %>% 
  dplyr::mutate(Group = factor(Group, levels = c(1:3), labels = paste0("Group ", c(1:3)))) %>%
  recall(., id = 1, opts = c(2:9), Group = 10, None = 9, choice = 11) %>% 
  dplyr::mutate_if(is.numeric, round, 2)
```
...
```{r}
specificity(data = HOT, id = 1, opts = c(2:9), choice = 11, None = 9)
```
...
Let us imagine you have a specific assortment, and you want to check how many participants would at least consider purchasing at least one of the products or alternatives you are testing in the function. 

To run this function you need to specify a threshold, i.e., the no-buy alternative. `validateHOT` includes two methods: `threshold` and `First Choice`. `threshold` considers all products / alternatives that pass the threshold as purchase option, while `First Choice` only marks the product / alternative with the highest utility as purchase option and marks it as purchase option in case its preferred over the no-buy alternative.

```{r}
reach(data = HOT, id = 1, bundles = c(2, 3, 7), None = 9, method = "threshold")
```

Let us see, how the reach changes when we use `First Choice` instead and give the results by Groups
...
Next, let us check the `freqAssort` function, which tells you how many products / alternatives on average are preferred over the no-buy alternative. Again, `validateHOT` offers the `threshold` and `First Choice` method. 
...
HOT %>% 
  dplyr::mutate(Group = factor(Group, levels = c(1:3), labels = paste0("Group ", c(1:3)))) %>%
  freqassort(., id = 1, bundles = c(2, 3, 7), None = 9, method = "First Choice", Group = 10) %>% 
  dplyr::mutate_if(is.numeric, round, 2)
```

### `shareofpref`

Finally, `validateHOT` also provides you the assumed share of preference of all products / alternatives you specify in your assortment

```{r}
HOT %>%
  shareofpref(data = ., id = 1, opts = c(2:9))
```
