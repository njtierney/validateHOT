...
In case, we first had change `Group` to a `factor`, the output would have been adapted accordingly. This is shown in the example in which we use `mae` in `tidyverse` logic.

```{r, message=F, warning=F}
library(tidyverse)

HOT %>% 
  mutate(Group = factor(Group, levels = c(1:3), labels = paste0("Group ", c(1:3)))) %>%
