...
After running the Hierarchical Bayes estimation in Sawtooth Software, the raw utility scores should be exported. Both the raw utilities and the actual choice in the validation/holdout task needs be read into *R* (please see the vignette for more advice). In the following, we provide the workflow for a MaxDiff study (the vignette also provides detailed examples for a CBC as well as an ACBC). 

We create the validation/holdout task, which has a total of 7 items (\texttt{\color{purple}prod}) plus the no-buy alternative (\texttt{\color{purple}none}). Moreover, we also define the method, the actual choice in the validation/holdout task as well as the column indexes of the alternatives in the validation/holdout task. Finally, we define variables that we would like to keep in the data frame (\colcod{varskeep}). \texttt{\color{purple}createHOT} returns a data frame which we need for subsequent analyses.
...
For the following example, let us assume that you conducted an anchored MaxDiff analysis with 10 items (\colcod{opts}) and you want to find the best assortment with a size of 3 (\colcod{size = 3}). As a threshold (\colcod{none}) you use the anchor (no-buy alternative). Finally, you apply the threshold approach, that is that each alternative with a utility above the threshold (in our case \colcod{none}) is marked as purchase option. We reduce the output to the Top 5 combos based on the reach (a \colcod{1} indicates present in this assortment while \colcod{0} means not present).
