...
#' to get \code{"specificity"} by group
...
#'
#' @details
#' The current logic of \code{"specificity"} is to provide whether a binary coded event is correctly predicted.
...
#' validation/holdout task. Needs to be specified for \code{specificity()}.
#'
#'
#' @return a data frame
#'
...
specificity <- function(data, id, Group = NULL, opts, choice, None) {
  ...
  if (base::is.null(Group)) {
    ...
    return(tibble::tibble(
      WS %>%
      dplyr::summarize(
        specificity = 100 * (base::sum(buy == 2 & pred_buy == 2) / (base::sum(buy == 2 & pred_buy == 2) + base::sum(buy == 2 & pred_buy == 1)))
      ))
    )
  }
