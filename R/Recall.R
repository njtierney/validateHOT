...
#'
#' @details
#' The current logic of \code{"recall"} is to determine whether a binary coded is correctly predicted by the model.
...
recall <- function(data, group, opts, choice, none) {
  ...
  return(data %>%
    ...
    dplyr::summarise(
      recall = 100 * (base::sum(buy == 1 & pred == 1) / (base::sum(buy == 1 & pred == 1) + base::sum(buy == 1 & pred == 2)))
    ))
}
