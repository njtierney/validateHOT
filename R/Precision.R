...
#'
#' @details
#' The current logic of \code{"precision"} is to determine whether a binary coded is correctly predicted by the model.
...
#'
#' # precision ungrouped
#' precision(data = HOT, opts = c(Option_1:None), choice = choice, none = None)
#'
#' # precision by group
#' precision(data = HOT, opts = c(Option_1:None), choice = choice, none = None, group = Group)
#' }
#'
#' @export
...
precision <- function(data, group, opts, choice, none) {
  ...
  return(data %>%
    ...
    dplyr::summarise(
      precision = 100 * (base::sum(buy == 1 & pred == 1) / (base::sum(buy == 1 & pred == 1) + base::sum(buy == 2 & pred == 1)))
    ))
}
