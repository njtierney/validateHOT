...
#' to get \code{"precision"} by group
...
#' validation/holdout task. Needs to be specified for \code{precision()}.
#'
#'
#'
#' @return a data frame
...
precision <- function(data, id, Group = NULL, opts, choice, None) {
  ...
  if (base::is.null(Group)) {
    ...
    return(tibble::tibble(
      WS %>%
      dplyr::summarise(
        precision = 100 * (base::sum(buy == 1 & pred_buy == 1) / (base::sum(buy == 1 & pred_buy == 1) + base::sum(buy == 2 & pred_buy == 1)))
      ))
    )
  }
