#' Precision
#'
#' @description precision is one of the 5 metrics of the confusion matrix
...
#'
#' @details
#' The current logic of \code{"precision"} is to determine whether a binary
#' coded is correctly predicted by the model.
#' To use the function a \code{"none"} alternative has to be included in the
#' validation/holdout task. One potential usage is, for example, whether a
#' buy or a no-buy condition was predicted correctly. For example,
#' you have three alternatives plus a \code{"none"} alternative and you
#' want to check whether a buy or no-buy was correctly predicted. This function
#' can be helpful when you test, for example, if your model significantly
#' overestimates or underestimates, for example, a purchase likelihood.
#'
